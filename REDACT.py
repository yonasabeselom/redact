# ╔══════════════════════════════════════════════════════════════════╗
# ║     REDACT  —  v1.0                                              ║
# ║     218 items · 4 tiers · Windows 11 Fluent Dark UI             ║
...
# ─── 218 Items Deep Forensic Matrix ───────────────────────────────────────────
ALL_ITEMS = [
    ...
    ("LOW","terminal_server_mru","Terminal Server Client Cache","Operational markers documenting interface tracking connections inside system links",lambda: [_reg("HKCU",r"Software\Microsoft\Terminal Server Client")]),
    ...
    ("MEDIUM","git_credential_cache","Git Operational Tracking","Local transaction history caches registering repository interact milestones",lambda: _clean(r"%USERPROFILE%\.gitconfig")),
    ...
    ("MEDIUM","winscp_log_history","WinSCP Operational Logs","Operational log tracking data documenting remote target adjustments and server links",lambda: _clean(r"%APPDATA%\WinSCP.log")),
    ("MEDIUM","cyberduck_host_history","Cyberduck Connection Profiles","Tracks structural profile parameters logging target domains and cloud parameters",lambda: _clean(r"%APPDATA%\Cyberduck\History\*")),
    ("MEDIUM","filezilla_mru_history","Filezilla Server History","Local configuration records listing recently accessed targets and site definitions",lambda: _clean(r"%APPDATA%\FileZilla\recentservers.xml")),
    ("MEDIUM","teamviewer_connections","TeamViewer Event Metrics","Operational trace files noting remote control access parameters and session tags",lambda: _clean(r"C:\Program Files\TeamViewer\TeamViewer*_Logfile.log")),
    ...
    ("HIGH","browser_indexeddb_extensions","Extension Staging Profiles","Wipes data environments staging secondary parameters utilized by extension modules",lambda: _wipe(r"%LOCALAPPDATA%\Google\Chrome\User Data\Default\IndexedDB\*", r"%APPDATA%\Mozilla\Firefox\Profiles\*\storage\default\*", r"%LOCALAPPDATA%\Microsoft\Edge\User Data\Default\IndexedDB\*")),
    ("HIGH","browser_media_history_sqlite","Browser Streaming Indexes","SQLite records documenting video streaming interactions processed inside browser frames",lambda: _wipe(r"%LOCALAPPDATA%\Google\Chrome\User Data\Default\Media History*", r"%LOCALAPPDATA%\Microsoft\Edge\User Data\Default\Media History*")),
    ...
    ("HIGH","ntfs_transaction_log","NTFS Transaction Log ($Txf)","System directory caching file system modifications before sector commitments",lambda: [_run('fsutil resource setautoreset true C:\\', "Reset transaction logs drive C")]),
    ...
    ("HIGH","cortana_core_db","Cortana Core Database","SQLite stores archiving local semantic metrics and interaction logs",lambda: _clean(r"%LOCALAPPDATA%\Packages\Microsoft.Windows.Cortana_*\LocalState\*.db")),
    ...
    ("HIGH","taskbar_badging_mru","Taskbar Notification Badges","Wipes transactional counters logging badge alert interactions over active apps",lambda: [_reg("HKCU",r"Software\Microsoft\Windows\CurrentVersion\Explorer\Taskband\Badging")]),
    ("HIGH","app_readiness_logs","AppReadiness Service Logs","Diagnostic log files monitoring third-party store application preparation instances",lambda: _clean(r"C:\Windows\System32\LogFiles\AppReadiness\*")),
    ("HIGH","win_web_platform_cache","Web Platform Staging Pool","Transient objects generated by web integration frameworks running behind modern layers",lambda: _clean(r"%LOCALAPPDATA%\Microsoft\Windows\WebPlatform\*")),
    ("HIGH","win_active_desktop","Active Desktop Layout Cache","Registry indicators configuring background rendering engine states",lambda: [_reg("HKCU",r"Software\Microsoft\Windows\CurrentVersion\Explorer\Wallpaper\MRU")]),
    ("HIGH","office_document_cache","Office File Cache Engine","Localized transactional databases backing local copies of network documents",lambda: _clean(r"%LOCALAPPDATA%\Microsoft\Office\16.0\OfficeFileCache\*")),
    ("HIGH","wer_machine_queue","Machine Queue Error Reports","System-wide fatal exception reports pending cloud evaluation loops",lambda: _clean(r"C:\ProgramData\Microsoft\Windows\WER\ReportQueue\*")),
    ("HIGH","wdi_boot_perf_logs","WDI Performance Snapshots","Forensic diagnostic records detailing boot execution sequences and module times",lambda: _clean(r"C:\Windows\System32\WDI\LogFiles\BootPerf\*")),
    ("HIGH","win_core_delivery_db","Delivery Optimization Core","Centralized indexing storage mapping background delivery service content distributions",lambda: _clean(r"C:\Windows\ServiceProfiles\NetworkService\AppData\Local\Microsoft\Windows\DeliveryOptimization\Cache\*")),
    ("HIGH","browser_extension_settings","Extension Context Databases","SQLite structural files capturing settings and preferences isolated to addon contexts",lambda: _wipe(r"%LOCALAPPDATA%\Google\Chrome\User Data\Default\Local Extension Settings\*", r"%LOCALAPPDATA%\Microsoft\Edge\User Data\Default\Local Extension Settings\*")),
    ("HIGH","win_credential_manager","Windows Vault Credentials","Cryptographic database rows capturing system web accounts and stored security profiles",lambda: [_run("cmdkey /list | ForEach-Object { if($_ -match 'target=(.*)') { cmdkey /delete:$matches[1] } }", "Clear system vault credentials (best-effort)")]),
    ("HIGH","explorer_bags_virtual","Virtual ShellBag Extensions","Registry keys capturing configuration attributes of non-file folders and layout objects",lambda: [_reg("HKCU",r"Software\Microsoft\Windows\CurrentVersion\Explorer\Bags")]),
]

assert len(ALL_ITEMS) == 218, f"Expected 218, got {len(ALL_ITEMS)}"
...
class App(tk.Tk):
    ...
    def _build_fluent_ui(self):
        ...
        tk.Label(text_header_f, text="Forensic Privacy Eraser & Disk Sanitization Utility · v1.0 · 218 System Items Active", font=FONT_FL_SUB, fg=W11_TEXT_MUTED, bg=W11_BG).pack(anchor="w", pady=(2, 0))
        ...
        self.lbl_metrics = tk.Label(ctrl_f, text="00 / 218 Targets Active", font=FONT_FL_BOLD, fg=W11_ACCENT, bg=W11_BG)
    ...
    def _update_selection_metrics(self):
        ...
        self.lbl_metrics.config(text=f"{n:02d} / 218 Targets Active")
    ...
    def _dispatch_engine(self, selected):
        ...
        log_report_lines = [
            ...
            f" Selected Clean Modules: {total} of 218",
        ...
        complete_target_filepath = os.path.join(desktop_path, f"REDACT_Execution_Log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
        
        try:
            with open(complete_target_filepath, "w", encoding="utf-8") as target_file:
                target_file.write("\n".join(log_report_lines))
            saved_notification = "Execution Log exported safely to Desktop."
